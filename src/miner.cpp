...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 9) { txNew.vout[0].nValue = 2100000 * COIN;} // Premine 21,000,000 * 10 = 21,000,000 xbit 10%
    if (prev->nHeight > 9) { txNew.vout[0].nValue = 100 * COIN;} // Block 9 to 2016 pay 100 xbit
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("xbitMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("xbitMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("xbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xbit-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running xbitMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
