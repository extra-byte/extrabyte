...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::xbit)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizexbitAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizexbitAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizexbitAmount = strAnonymizexbitAmount.remove(strAnonymizexbitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexbitAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizexbitAmount * COIN) nMaxToAnonymize = nAnonymizexbitAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizexbitAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizexbitAmount));
        strAnonymizexbitAmount = strAnonymizexbitAmount.remove(strAnonymizexbitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexbitAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
